...
def render_s2_table(rows):
  ...
  <th class="num" onclick="sortTable(this)">period_start 收盤</th>
...
def render_s3_table(rows):
  ...
  <th class="num" onclick="sortTable(this)">period_end+1 開盤</th>
  <th class="num" onclick="sortTable(this)">最新價</th>
  <th class="num" onclick="sortTable(this)">區間報酬</th>
...
<div id="pane-s2" class="pane active">
  ...
  <div class="desc">
    歷史回測定義：以 period_start 收盤為起算點、period_end 收盤為終點的區間報酬統計。
  ...
  <div class="box">
    <b>名詞定義：</b><br/>
    • <b>進場日量比</b> = period_start 當日成交量 ÷ 該股 period_start 前 10 個交易日的平均成交量<br/>
    &nbsp;&nbsp;&nbsp;例：量比 0.19 表示該日成交量僅為前 10 日均量的 19%（大幅量縮）；2.0 表示翻倍（爆量）<br/>
    • <b>紅K</b> = period_start 當日收盤價 &gt; 開盤價<br/>
    • <b>進場日漲幅</b> = (period_start 收盤 − period_start 開盤) ÷ period_start 開盤 × 100%<br/>
    • <b>區間報酬</b> = (最新收盤 − period_start 收盤) ÷ period_start 收盤 × 100%<br/>
    • <b>區間最大回撤</b> = (period_start 至今最低收盤 − period_start 收盤) ÷ period_start 收盤 × 100%
  </div>
...
<div id="pane-s3" class="pane">
  ...
  <div class="desc">
    歷史回測定義：period_end + 1 個交易日開盤為起算點、再 10 個交易日後收盤為終點的區間報酬統計。
...
<div id="pane-about" class="pane">
  ...
  <div class="desc" style="margin-top:14px">
    ...
    <b>S2 處置期間區間樣本</b><br/>
    回測定義：period_start 收盤 → period_end 收盤<br/>
    回測（2024-04 ~ 2026-04，1316 處置事件，還原權息價）：<br/>
    ...
    回測定義：period_end + 1 開盤 → period_end + 10 收盤<br/>
    回測：N=904，勝率 48.1%，平均 +3.55%，PF 1.63，σ 20.2%<br/>
    &nbsp;&nbsp;• 中位數 -0.62%，典型肥尾分布<br/><br/>

    <b>歷史回測未顯示 edge 的策略：</b><br/>
    S1 進處置前動能延續（公告隔日進、處置生效日出）：勝率 37%，平均 -0.97%，PF 0.67<br/><br/>

    <b>方法學注意：</b>所有回測未含手續費及滑價。處置股因人工管制撮合（5 或 20 分鐘一次），
