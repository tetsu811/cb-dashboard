...
VOLUME_SURGE_THRESHOLD = 1.5  # 當日量能 ≥ 20 日均量 x 此倍數視為放量
...
def fetch_stocks_bars(symbols):
    ...
    for sym in symbols:
        try:
            ...
            vol_avg = bars["Volume"].iloc[-21:-1].mean() if len(bars) >= 21 else bars["Volume"].mean()
            vol_ratio = (vol_today / vol_avg) if vol_avg else None
            out[sym] = {
                "close": float(last),
                "day_pct": float(day_pct),
                "vol_ratio": float(vol_ratio) if vol_ratio else None,
...
def build_sector_report(sector_code, sector_name_zh, sector_name_en, bars_df):
    ...
    gainers = contributions[:CONTRIB_SHOW]
    losers = list(reversed(contributions[-CONTRIB_SHOW:])) if len(contributions) >= CONTRIB_SHOW else []

    # 放量個股（不論漲跌）
    surges = [c for c in contributions if c["vol_ratio"] and c["vol_ratio"] >= VOLUME_SURGE_THRESHOLD]
    surges.sort(key=lambda x: x["vol_ratio"] or 0, reverse=True)
...
def fetch_news_and_earnings(symbols):
    """並行抓新聞 + 財報。回傳 (news_map, earnings_map)。"""
    symbols = sorted(set(symbols))
    if not symbols:
        return {}, {}
    print(f"抓取 {len(symbols)} 檔個股的新聞與財報...")
    news_map, earn_map = {}, {}
    ...
    print(f"  新聞總數 {total_news}，財報事件個股數 {total_earn}")
    return news_map, earn_map


def attach_news_and_earnings(sectors, news_map, earn_map):
    """把每個板塊 top contributors 的新聞/財報掛到板塊資料上。"""
    for s in sectors:
        if s.get("status") != "ok":
            continue
        ...
        for sym in top_syms:
            for item in news_map.get(sym, []):
                ...
                seen_titles.add(key)
                news.append(item)
        news.sort(key=lambda x: x["ts_epoch"], reverse=True)
        s["news"] = news[:NEWS_PER_SECTOR]
...
def _gen_contrib_list(items, is_positive):
    if not items:
        return '<div class="empty-msg" style="font-size:12px">無資料</div>'
    html = ""
    for c in items:
        ...
        vol_tag = ""
        if c.get("vol_ratio") and c["vol_ratio"] >= VOLUME_SURGE_THRESHOLD:
            vol_tag = f' <span class="pill am" title="量能 {c["vol_ratio"]:.2f}x 20 日均量">放量</span>'
        html += f'''<div class="row-item">
  <span class="sym">{c["symbol"]}</span>
  <span class="nm" title="{c["name"]}">{c["name"]}{vol_tag}</span>
  <span class="val {cls}">{_fmt_pct(c["day_pct"])}</span>
  <span class="val mu" style="color:var(--mu);font-weight:500;min-width:42px" title="權重 {weight_pct:.2f}%">{weight_pct:.1f}%</span>
</div>'''
    return html
...
def _gen_surge_list(items):
    if not items:
        return '<div class="empty-msg" style="font-size:12px">今日無明顯放量個股</div>'
    html = ""
    for c in items:
        cls = "up" if c["day_pct"] >= 0 else "dn"
        vr = c["vol_ratio"] or 0
  ...
  <span class="val am">×{vr:.2f}</span>
  <span class="val {cls}" style="min-width:52px">{_fmt_pct(c["day_pct"])}</span>
</div>'''
    return html
...
def main():
    ...
    for s in sectors:
        ...
        for c in s.get("all_holdings", []):
            enrich_syms.add(c["symbol"])
    news_map, earn_map = fetch_news_and_earnings(enrich_syms)
    attach_news_and_earnings(sectors, news_map, earn_map)
