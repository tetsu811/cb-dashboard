...
# Model 分工（cost 優化）：
# - Haiku 4.5：budget 結構化抽取（純結構化任務，Haiku 夠用、便宜 5x）
# - Sonnet 4.6：公司映射（需廣泛美股知識與判斷，Sonnet 質量夠）
# - 通過機率：純 rule-based，不打 Claude（已包含階段基準 + 兩黨/必過 boost）
...
# Pipeline 版本。改動 Claude prompt 或抽取邏輯時 bump 這個，cache 中版本不符的會強制重跑。
PIPELINE_VERSION = "v0.2"

# 抓 bill text 時最大字元數。~15K chars ≈ ~3.8K tokens，input cost 可控。
BILL_TEXT_MAX_CHARS = 15000

# Policy areas that map to investable industries (Congress.gov 官方分類)
# 高價值：幾乎一定打 Claude；低價值：需要同時有階段進展或金額 keyword 才打
HIGH_VALUE_POLICY_AREAS = {
    "Appropriations", "Armed Forces and National Security", "Energy",
    "Health", "Science, Technology, Communications",
    "Transportation and Public Works",
}
POLICY_AREAS_OF_INTEREST = HIGH_VALUE_POLICY_AREAS | {
...
def fetch_bill_text_excerpt(congress, btype, number, max_chars=BILL_TEXT_MAX_CHARS):
    """抓 bill text，抽含金額的段落（Congress.gov summary 通常不含具體金額）。"""
    data = cg_get(f"/bill/{congress}/{btype}/{number}/text")
    if not data or not data.get("textVersions"):
        return ""
    versions = data["textVersions"]
    # 最新版本
    versions = [v for v in versions if v.get("formats")]
    if not versions:
        return ""
    versions.sort(key=lambda v: v.get("date") or "", reverse=True)
    latest = versions[0]
    # 偏好 Formatted Text (易 parse)，其次 Formatted XML
    formats = latest.get("formats", [])
    url = None
    for prefer in ("Formatted Text", "Formatted XML"):
        for fmt in formats:
            if fmt.get("type") == prefer:
                url = fmt.get("url")
                break
        if url:
            break
    ...
    # 按段落切（較寬鬆）
    paragraphs = re.split(r"(?:\. {2,}|SEC\.? \d+|TITLE [IVXL]+)", clean)
    relevant = []
    for p in paragraphs:
        p = p.strip()
        if len(p) < 30:
            continue
        # 只保留含金額、撥款、授權金額的段落
        if re.search(r"\$[\d,]{4,}|\b(?:appropriat\w+|authoriz\w+)\b.{0,200}\$|(?:billion|million) dollars?", p, re.IGNORECASE):
            relevant.append(p[:800])  # 單段上限
    if not relevant:
        # 沒抽到金額段，退回全文前段（有時關鍵字不在我們的 regex 內）
...
def _action_shows_progress(action_text):
    """從 latestAction.text 判斷是否有實質進展，節省 Claude call。"""
    t = (action_text or "").lower()
    return bool(re.search(
        r"passed|agreed to|reported by|ordered to be reported|markup|"
        r"became public law|presented to president|placed on.*calendar",
        t
    ))
...
BUDGET_EXTRACT_SYSTEM = """你是美國聯邦預算法案分析師。給定法案的【官方摘要】與【法案內文含金額段落】，抽取結構化的預算資訊。

**關鍵原則**：Congress.gov 的官方摘要通常是敘述性的（只描述法案在做什麼），具體金額通常在 **內文段落** 裡。你要主動從內文的 "$X,XXX,XXX" 或 "$X million" / "$X billion" / "shall be appropriated $X" / "authorized to be appropriated" 等 pattern 抓金額。

判斷重點：
1. 總預算 (total_amount_usd)：整份法案的核心金額。多年計畫回報累計總額。若內文有多個分項金額，相加得到 total。
2. 實施年限 (period_years)：看內文是否出現 "fiscal years 2025 through 2029" 這類句型。單年或不明填 null。
3. 各 program：列出主要支出項目與金額（從內文段落抽取）。
4. 授權 vs 撥款：
   - Appropriation 撥款法案（bill title 含 "Appropriations Act"）= 實際撥錢，is_appropriation=true
   - Authorization 授權法案 = 只核准上限，實際是否撥款看後續 appropriation，is_appropriation=false 並在 notes 標注「授權金額非實際撥款」
5. 若內文與摘要都無具體金額 → total_amount_usd=null, confidence<0.3, notes 說明為何找不到。
...
def map_companies(title, budget_json, summary):
    """使用 Sonnet 4.6 — 需要對美股有廣泛知識做判斷，Sonnet 質量夠。"""
    def call():
        resp = claude.messages.parse(
            model=MODEL_SONNET,
            max_tokens=4000,
...
def process_bill(bill_ref, cache):
    ...
    # 抓 bill text 中含金額的段落（Congress.gov 的 summary 通常只有敘述，金額在 text 裡）
    text_excerpt = fetch_bill_text_excerpt(detail["congress"], detail["type"], detail["number"])

    combined_input = ""
    if summary_text:
        combined_input = f"【官方摘要】\n{summary_text}"
    if text_excerpt:
        combined_input += f"\n\n【法案內文含金額段落】\n{text_excerpt}"
    if not combined_input:
        combined_input = f"(尚無摘要與文本) {title}"

    # Claude: extract budget
    budget = extract_budget(combined_input, title)
    ...
    # 通過機率：純 rule-based（省一次 Claude call）
    baseline = STAGE_BASELINE.get(stage, 0.04)
    passage_prob = compute_passage_prob(detail, stage, 0.0)
    ...
    if budget and budget.total_amount_usd:
        ...
        mapping = map_companies(title, budget_json, combined_input)
    ...
    record = {
        ...
        "passage_nuance_adj": 0.0,
        "passage_nuance_reason": "",
...
<div class="kv">
  ...
  <div><span class="k">通過機率</span><span class="v">{prob*100:.1f}%</span>
    <span style="font-size:11px;color:var(--mu);margin-left:6px">(階段基準 {rec["passage_baseline"]*100:.0f}% + 兩黨/必過 rule-based 加權)</span></div>
